...
packages <- c("ggplot2", "vioplot", "gridExtra", "scales", "randomcoloR", "glmmTMB", "performance", "DHARMa", "buildmer", "ggeffects", "car", "emmeans", "multcomp", "vegan", "ade4", "factoextra")
...
p_discri_Bface <-ggplot(df_discri, aes(x=Axis1, y=Axis2, color=Bface)) + 
               geom_point(size=3) + 
               stat_ellipse(aes
               (x=Axis1, y=Axis2, color=Bface), type = "norm", level=0.95, lwd=1.25) +
...
p_discri_zone <-ggplot(df_discri, aes(x=Axis1, y=Axis2, color=zone)) + 
               geom_point(size=3) + 
               stat_ellipse(aes
               (x=Axis1, y=Axis2, color=zone),type = "norm", level=0.95, lwd=1.25) +
               labs(
...
p_discri_species <- ggplot(df_discri, aes(x=Axis1, y=Axis2, color=Bface)) + 
  labs(
   ...
   aes(x = 0, y = 0, 
       xend = (Axis1*mult1), yend = (Axis2*mult1)), 
       arrow = arrow(length = unit(1/2, "picas")),
       color = "black",
       lwd=1) +
   annotate(
       "text", x = (discri_sp$Axis1*(mult1+0.05)), y = (discri_sp$Axis2*(mult1+0.05)),
       label = species_names) +
       xlim(-0.35, 0.32) +
       ylim(-0.32, 0.32)
p_discri_species
